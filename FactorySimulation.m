...
N = 6; % number of nodes(targets)
ts = 0.1; T = 20; e=0.1; % time set to simulation
g = [10 10 0]; 
...
for i = 1:R    
    while 1
        ...
        if( all(d>0.2)) % robot의 초기 위치는 서로 20cm이상 떨어지게 해놓음
            break
        end
    end
end    
...
for t = 0:ts:T
    index = index + 1;
    %node call set
    n = nodeCall(n,t_record);
    %robot velocity set
    %[p, q, n] = RobotTaskAllocation_test(p, q, n, g, ts);
    ...
    for i = 1:N
        if n(i,3) == 0 
            n_plot = plot(n0(i,1),n0(i,2),'go','MarkerSize',8);
        else
            n_plot = plot(n0(i,1),n0(i,2),'g*','MarkerSize',8);
        end
    end
    title(['Factory simulation - ' num2str(t) '[sec]'])
    p_vec(:,:,i) = p;
    n_vec(:,:,i) = n;
    drawnow;
    frame = getframe(gca);
